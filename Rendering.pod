...
Dieses Dokument erklärt die Inhaltsschaffung mit dem L<Mojolicious> Renderer.
...
Der Renderer ist eine kleine Blackbox, die gespeicherte Daten unter Einbeziehung
verschiedener Templatesystme und Datenkodierungsmodulen in tatsächliche
...
Templates können, wenn vom Entwickler oder den Routen genügend Informationen
bereitgestellt werden automatisch erkannt werden.

Es wird davon ausgegangen, dass Templatenamen dem C<name.format.handler>
Schema folgen. C<name> zeigt standardmäßig auf C<controller/action> oder den
Routennamen und C<format> zeigt auf C<html> sowie C<handler> auf C<ep>.
...
der C<DATA> Sektion der Klasse C<main> liegen.
...
Der Renderer kann mit Plugins einfach erweitert werden, um zusätzliche Templatesysteme
zu unterstützen, dazu später mehr.
...
der Zeilen stehen, nutzt.
  ...
  % Perl Code Zeilen, behandelt als "<% line =%>"
  %= Perl Ausdruck Zeile, behandelt als "<%= line %>"
  %== Perl Ausdruck Zeile, behandelt als "<%== line %>"
  %# Kommentarzeile, behandelt als "<%# line =%>"
  %% Ersetzt durch "%", nützlich um Templates zu generieren
...
Ein weiteres Gleichheitszeichen kann zum Ausstreichen von den Zeichen
C<E<lt>>, C<E<gt>>, C<&>, C<'> and C<"> in den Ergebnissen der Perl Ausdrücke
benutzt werden. Dies ist Standard um XSS Atacken gegen deine Anwendung zu
verhindern.
...
Du kannst auch ein weiteres Gleichheitszeichen am Ende eines Tags hinzufügen
um alle umgebenden Leerzeichen entfernen zu lassen. Das ermöglicht dir freies
Einrücken ohne das Ergebnis zu ruinieren.

  <% for (1 .. 3) { %>
    <%= $foo =%>
  <% } %>

Stash-Werte, die keine ungültigen Zeichen in ihren Namen enthalten,
...
es automatisch aufgerufen wird, falls noch nichts gerendert wurde,
sobald der Routen-Dispatcher seine Arbeit beendet hat.
Dies bedeutet, dass du auch nur auf Templates zeigende Routen haben kannst,
die keine tatsächlichen Aktionen haben.

  $self->render;

Aber es gibt noch einen großen Unterschied, indem du C<render> manuell
aufrufst, kannst du sicher gehen, dass das Template die aktuelle
Kontrollerinstanz und nicht den Standardkontroller, der im C<controller_class>
Attribut der Anwendungsklasse festgelegt wurde.

=head2 Templates rendern (C<template>)

Der Renderer wird immer versuchen das richtige Template zu erkennen. Du kannst
jedoch auch den C<template> Stashwert benutzern um ein bestimmtes zu rendern.

  $self->render(template => 'foo/bar');

Ein bestimmtes C<format> und C<handler> zu nutzen ist genauso einfach.
...
Da die automatische Erkennung auf einem Pfad beruht, muss du möglicherweise
auch einen C<handler> übergeben.
...
Der C<json> Stashwert erlaubt es dir Perl Strukturen an den Renderer zu
...
Diese Mappings können mit L<Mojolicious/"types"> einfach geändert werden.
...
=head2 Inhaltsnegoziierung

Für Ressourcen mit verschiedenen Repräsentationen, dich auch eine tatsächliche
C<RESTful> Inhaltsbeschaffenheit fordern kannst du auch C<respond_to> anstelle
...
Callbacks können für Repräsentationen genutzt werden, die zu komplex sind um
in einen einzelnen C<render> Aufruf zu passen.
...
Fallback genutzt oder einfach eine leere C<204> Antwort automatisch gerendert.
...
und C<tag helpers>, die Templatespezifisch sind und eher zum C<HTML>-Tag
generieren genutzt werden.
...
Inhalte in ein HTML Skelett einlegen wollen. Dank Layouts ist das überhaupt
kein Problem.
...
Du wählst einfach das passende Layout mit dem C<layout> Helper und platzierst
das Ergebnis des aktuellen Templates mit dem C<content> Helper.
...
Wie die meisten Helfer ist C<include> nur ein Shortcut um dein Leben einfacher
zu machen.
...
Anstelle von C<inclide> kannst du auch den C<render> mit dem C<partial>
Argument aufrufen.
...
an C<include> übergibst, werden diese automatisch lokalisiert und nur im
teilweisen Template verfügbar sein.
...
Ein Skelett für eine komplett C<CPAN> kompatibles Plugin-Distribution
kann automatisch generiert werden.

  $ mojo generate plugin DebugHelper

Und wenn du ein C<PAUSE> Account hast (welches du unter L<http://pause.perl.org>
erstellen kannst), bist du nur noch ein paar Kommandos davon entfernt es
zu C<CPAN> zu releasen.
...
=head2 Spezifische C<exception> und C<not_found> Templates
...
Daher probiert L<Mojolicious> immer zuerst C<exception.$mode.$format.*>
...
praktisch für Comet (C<long polling>) sein.
Jedoch funktioniert dies möglicherweise aufgrund der Limitationen einiger
...
=head2 Aufblasende DATA Templates

Templates, die in Dateien gespeichert sind werden Dateien in dem C<DATA>
...
aus dem C<DATA> Bereich in tatsächliche Dateien in den C<templates> und
C<public> Verzeichnissen.
...
L<Mojo::Template> enthält die komplette Liste der verfügbaren Optionen.

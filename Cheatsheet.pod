...
Embedded-Anwendung, an die weitergeleitet wird.
...
Ändere Raw-Bytes in eine Antwort um.
...
Basispfad zum Weiterleiten an embeddede Anwendungen.
...
Ändert Zeichen in eine Antwort.
...
Merke: L<IO::Socket::SSL> muss für den TLS-Support installiert sein.

  MOJO_CA_FILE=/etc/tls/cacerts.pem

=head2 C<MOJO_CERT_FILE>

Pfad zum TLS-Zertifikat, sollte immer einen Pfad wie C</etc/tls/client.crt>
sein.
Merke: L<IO::Socket::SSL> muss für den TLS-Support installiert sein.
...
I/O Operationen, verbraucht aber auch mehr Speicher. Standardmäßig C<131072>.

  MOJO_CHUNK_SIZE=1024

=head2 C<MOJO_CONFIG>

Konfigurationsdatei, dass von L<Mojolicious::Plugin::Config> und
L<Mojolicious::Plugin::JSONConfig> genutzt werden soll. Ziemlich nützlich zum
Testen.
...
Home-Directory der L<Mojolicious> Anwendung, sollte immer einen Pfad
...
Pfad zum TLS-Key, sollte immer einen Pfad wie C</etc/tls/client.key>
sein.
Merke: L<IO::Socket::SSL> muss für den TLS-Support installiert sein.
...
Maximale Buffergröße für durchgeleitete HTTP-Anfragen.
...
Maximale Zeilengröße von HTTP-Nachrichten Startzeilen und Headern in Bytes.
...
Maximale Größe für HTTP-Content, dass im Speicher gehalten wird.
...
Betriebsart der L<Mojolicious> Anwendung, sollte einen gültigen Modes wie
...
Deaktiviert Bonjour-Support.
Merke: L<Net::Rendezvous::Publish> muss für den Bonjour-Support installiert sein.
...
Deaktiviert IPv6-Support.
Merke: L<IO::Socket::IP> muss für den IPv6-Support installiert sein.

  MOJO_NO_IPV6=1

=head2 C<MOJO_NO_TLS>

Deaktiviert TLS Support, dies führt möglicherweise zu einer etwas
besseren Perfomanz und reduziertem Speicherverbrauch.
Merke: L<IO::Socket::SSL> muss für den TLS-Support installiert sein.
...
Aktiviert Reverse-Proxy Support für die L<Mojolicious> Anwendung.
...
Verzeichnis für temponäre Dateien, wie zum Beispiel größere Uploads.
Standardmäßig wird ein zufälliges, plattformabhängiges Verzeichnis
gewählt.
